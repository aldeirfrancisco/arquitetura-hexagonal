...
export default async function Polimorfismo() {
    ...
    const [tipoCarro] = await TerminalUtil
          .selecao('Tipo de carro?',['Ferrari', 'Fusca'])

    const carro: Carro = tipoCarro === 0 ? new Ferrari() : new Fusca();

    while(true){
     TerminalUtil.limpar()
     TerminalUtil.exibeChaveValor(
         "Velocidade Máxima: ",
     ...
     TerminalUtil.exibeChaveValor(
        ...
        const [opcao] = await TerminalUtil
          .selecao("Qual opção?", ["Acelerar", "Frear"]);
